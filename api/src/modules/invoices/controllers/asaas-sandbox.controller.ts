...
export class AsaasSandboxController {
  ...
  async confirmPayment(@Param('providerPaymentId') providerPaymentId: string) {
    ...
    try {
      return await this.asaasService.confirmSandboxPayment(providerPaymentId);
    } catch (error) {
  ...
  private assertSandboxEnabled() {
    ...
    if (process.env.NODE_ENV === 'production') {
      throw new ForbiddenException('Asaas Sandbox operations are disabled in production');
    }

    if (process.env.ASAAS_MOCK === 'true') {
      throw new BadRequestException('Asaas Sandbox operations require ASAAS_MOCK=false');
    }

    if (!asaasUrl.includes('api-sandbox.asaas.com')) {
      throw new ForbiddenException('Asaas Sandbox operations require ASAAS_URL to point to sandbox');
    }
  }
  ...
  private mapProviderError(error: unknown): never {
    ...
    const message =
      error instanceof Error ? error.message : 'Asaas Sandbox confirmation failed';
