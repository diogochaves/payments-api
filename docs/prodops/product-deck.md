...
Payments e o `System of Record for Payments` da Magazine Siara: a fonte oficial, auditavel e reconciliavel sobre invoices, cobrancas, confirmacoes, cancelamentos, recebimentos e eventos financeiros associados aos pedidos. Provedores como Asaas devem ser tratados como `PSP` (`Payment Service Provider`), ou seja, integracoes externas que executam cobrancas e enviam eventos, mas nao governam a verdade canonica do pagamento para o varejo. Ver definicao completa em [Payments, SOR e PSP](./payments-sor-psp.md).
